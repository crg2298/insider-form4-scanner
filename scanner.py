...
def main():
    ...
    <div class="card">
      ...
      <div class="item muted">• Insider transactions are only disclosed via Form 4</div>
    ...
    takeaway = (
        "Insider participation remains limited while analyst activity persists, suggesting "
        "elevated uncertainty and the need for selective positioning."
        if not hits else
        "Insider accumulation is present, with clustering indicating targeted conviction rather than broad risk-on behavior."
    )

    blocks.append(f"""
    <div class="card">
      <div class="section-title">📌 Trader Takeaway</div>
      <div class="item"><strong>Insider–Analyst divergence:</strong> {divergence}</div>
      <div class="item">{takeaway}</div>
    </div>
    """)
